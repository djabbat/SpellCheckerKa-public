defmodule SpellCheckerKaWeb.Plugs.RateLimiter do
  @moduledoc """
  Per-IP rate limiter for /api/check.
  Allows up to 30 requests per minute per IP address.
  ETS table :rate_limit is initialized in SpellCheckerKa.Application.
  """
  ...
  def call(conn, _opts) do
    ...
    case check_rate(ip, now) do
      :ok ->
        # Check daily freemium quota
        admin_token = get_req_header(conn, "x-admin-token") |> List.first()

        case SpellCheckerKa.UsageTracker.check_and_record(ip, admin_token) do
          {:ok, _remaining} ->
            conn
          ...
          {:quota_exceeded, 0} ->
            ...
            |> send_resp(402, Jason.encode!(%{
              error: "ლიმიტი ამოიწურა",
              payment_required: true,
              upgrade_url: "https://spellcheckerka.drjaba.com/upgrade",
              free_minutes: 30,
              message: "დღიური 30-წუთიანი უფასო კვოტა ამოიწურა. გთხოვთ, შეიძინოთ სერვისი."
            }))
            |> halt()
        end
